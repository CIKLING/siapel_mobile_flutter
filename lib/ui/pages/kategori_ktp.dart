...
class KategoriKTP extends StatefulWidget {
  const KategoriKTP({Key? key}) : super(key: key);

  @override
  State<KategoriKTP> createState() => _FormLayananKTP();
}

class _FormLayananKTP extends State<KategoriKTP> {
  ...
  final Map<int, String> produkItems = {
    ...
    113: 'KTP HILANG',
    114: 'KTP RUBAH',
    115: 'KTP PEREKAMAN(KANTOR DISPENDUK)',
  };
  ...
  Future<void> _kategoriktp() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        FormData formData = FormData.fromMap({
          ...
          'img3':
              await validateAndPrepareFile(selectedFiles['Upload Kehilangan']),
          'selfie':
        ...
        final response = await dio.post(
          'http://10.101.1.154:8000/ktp/create',
          data: formData,
          options: Options(
  ...
  Widget inputSection() {
    ...
    Widget kehilangan(String judul) {
      return Container(
        margin: EdgeInsets.only(bottom: 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
